...
Reads (whatever exists):
  reports/financebench_casestudy_casestudy.json   (diagnose + fix arc + LLM baseline)
...
Run:  python examples/financebench_casestudy/make_report.py   (no PYTHONPATH/env needed)
"""
...
def main():
    cs = _load(os.path.join(REPORTS, "financebench_casestudy_casestudy.json"), {})
...
All numbers regenerated by <span class="mono">financebench_casestudy.run_casestudy</span>.</p>
...
PYTHONPATH=examples python -m financebench_casestudy.run_casestudy     # diagnosis + fix arc + LLM baseline
PYTHONPATH=examples python -m financebench_casestudy.compare_fix_order # target-selection comparison
python examples/financebench_casestudy/make_report.py                 # regenerate this HTML</pre>
