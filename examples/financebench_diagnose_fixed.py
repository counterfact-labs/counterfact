...
8-agent financial RAG pipeline with FIXED prompts.
Applied 3 prompt edits based on counterfact diagnosis:
1. Context enricher: only reference source data
2. Fact checker: explicit $1M tolerance, error-seeking framing
3. Tone editor: preserve exact figures
...
def context_enricher(state: State) -> dict:
    """FIXED: Only reference data from the source document."""
...
def fact_checker(state: State) -> dict:
    """FIXED: Explicit tolerance, error-seeking framing."""
...
def tone_editor(state: State) -> dict:
    """FIXED: Preserve exact figures and units."""
    analysis = state.get("analysis", "")
    if not analysis:
        return {}
